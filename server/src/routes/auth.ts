...
router.post("/login/forgot", (req, res) => {
  ...
  User.findOne({ email: req.body.email }, function (err: Error, user: IUser) {
    if (err) {
...
router.post("/login/reset/:token", (req, res) => {
  ...
  Token.findOne({ token: req.params.token }, function (err: Error, token: IToken) {
    if (err) {
    ...
    User.findById(token._userId, function (err: Error, user: IUser) {
      if (err) {
        return res.status(500).send("An unexpected error occurred");
      }
...
router.post("/register", async (req, res) => {
  ...
  let user = await User.findOne({ username: req.body.username.toLowerCase() }, function (err: CallbackError) {
    if (err) {
      return res.status(500).send("An unexpected error occurred");
    }
  });
  ...
  user = await User.findOne({ email: req.body.email.toLowerCase() }, function (err: CallbackError) {
    if (err) {
      return res.status(500).send("An unexpected error occurred");
    }
  });
      ...
      if (err) {
        ...
        token.save(function (err) {
          ...
          sgMail
            ...
            .catch(() => {
              User.findOneAndDelete({ email: user?.email, isVerified: false }, 
              //   function (err: any) {
              //   if (err) {
              //     return res
              //       .status(500)
...
router.post("/register/reset", (req, res) => {
  ...
  User.findOneAndDelete({ email: req.body.email, isVerified: false }, 
